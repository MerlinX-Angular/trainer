...
export class WorkoutComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
     // iš activatedRoute ištraukiamas egzistuojančios treniruotės pavadinimas ir lyginamas su esamu pavadinimu
    // this.queryParamsSub = this.activatedRoute.params.subscribe(params => this.workoutName = params['id']);
    // iš activatedRoute ištraukiama esama treniruotė ir sekama kada ji pasikeis į kitą
    this.sub = this.activatedRoute.data.subscribe(
         (data: { workout: WorkoutPlan }) => { this.workout = data.workout; }
    );
  }
  ...
  removeExercise(exercisePlan: ExercisePlan) {
    // jeigu buvo iškviestas metodas - true
  ...
  save(formWorkout: any) {
    ...
    this.workoutBuilderService.save().subscribe(
      success => this.router.navigate(['/builder/workouts']), err => console.error(err)
    );
  }
  ...
  deleteWorkout(name) {
    this.workoutService.removeWorkout(name).subscribe(
