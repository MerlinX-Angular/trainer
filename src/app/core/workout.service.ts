...
export class WorkoutService {
    ...
    workout: WorkoutPlan;
    collectionsUrl = 'https://api.mongolab.com/api/1/databases/trainer/collections';
    apiKey = '-TWYRGoaGSALSAL_Uaw4nbAXt403Xb2u';
    ...
    getByUniqueName(uName: string) {
        return this.http.get<any[]>(`${this.collectionsUrl}/workouts?q={"_id":"${uName}"}${this.params1}`);
    }
    ...
    getWorkout(workoutName: string): Observable<WorkoutPlan> {
        // leidžia dirbti su keliomis HTTP užklausomis vienu metu
      return forkJoin (
          // atvaizduoja visą sąrašą pratimų
          this.http.get(this.collectionsUrl + '/exercises' + this.params),
          // atvaizduoja treniruotės detales
          this.http.get(this.collectionsUrl + '/workouts/' + workoutName + this.params))
          .pipe(
              // sukuriame WorkoutPlan objektą su teisingai nustatytu pratimų masyvu
              // susiejame serverio modelio duomenis su mūsų klientų modeliu
               map(
                  (data: any) => {
      ...
      // sukuriame naują pratimų masyvą ir pridedame į serverį naujo pratimo pavadinimą ir trukmę
      addWorkout(workout: WorkoutPlan) {
        ...
        // sukuriame pranešimo body su identifikatoriumi, kuris bus pratimo pavadinimas
        const body = {
            '_id': workout.name,
