...
describe('WorkoutService', () => {
  const collectionUrl = 'https://api.mongolab.com/api/1/databases/trainer/collections';
  const apiKey = '-TWYRGoaGSALSAL_Uaw4nbAXt403Xb2u';
  ...
  afterEach(() => {
    // įsitikiname ar įvykdžius kiekvieną testą daugiau nėra jokių užklausų
  ...
  // ar galime įdiegti HttpClient kai sukuriame servisą
...
it('should should return all workout plans', () => {
  // sukuriame du masyvus ir tikriname ar jie sutampa
  ...
  // jeigu bus neatitikimų antras parametras fail grąžins klaidą 
   workoutService.getWorkouts().subscribe(
    workouts => actualWorkouts = workouts,
    fail
  );
  // ar užklausa bus vykdoma su teisingu URL adresu
